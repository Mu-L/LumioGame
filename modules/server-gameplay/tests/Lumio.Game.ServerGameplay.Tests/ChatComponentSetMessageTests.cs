...
public sealed class ChatComponentSetMessageTests
{
    ...
    public void ValidChatInputUpdatesExactlyOneSenderComponentAtNextFixedTick()
    {
        ...
        Assert.Equal(
            ...
            ChatSetMessageSystem.Admit(manager, "room-01", senderA, "C1", 1UL, new ChatInput("gg")).Kind);
    ...
    public void EventAndComponentStateCarryTheSameAppliedTick()
    {
        ...
        Assert.Equal(
            ChatOperationKind.Admitted,
            ChatSetMessageSystem.Admit(manager, "room-01", sender, "C1", 1UL, new ChatInput("hello")).Kind);
        manager.Tick();
    ...
    public void NetworkThreadSetMessageRejectsWithZeroComponentWrite()
    {
        ...
        Assert.Equal(
            ChatOperationKind.Admitted,
            ChatSetMessageSystem.Admit(manager, "room-01", sender, "C1", 1UL, new ChatInput("keep")).Kind);
        manager.Tick();
    ...
    public void NetworkThreadAdmitQueuesWithoutWritingUntilOwnerTick()
    {
        ...
        var worker = new Thread(() =>
        {
            admitted = ChatSetMessageSystem.Admit(manager, "room-01", sender, "C1", 1UL, new ChatInput("gg"));
        });
        worker.IsBackground = true;
        worker.Start();
        Assert.True(worker.Join(TimeSpan.FromSeconds(5)));
